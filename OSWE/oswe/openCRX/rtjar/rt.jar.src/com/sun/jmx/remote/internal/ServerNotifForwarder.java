...
/*     */     try {
/*     */       boolean bool = ((Boolean)AccessController.<Boolean>doPrivileged(new PrivilegedExceptionAction<Boolean>()
/*     */           {
/*     */             public Boolean run() throws InstanceNotFoundException {
/*     */               return Boolean.valueOf(ServerNotifForwarder.this.mbeanServer.isInstanceOf(name, ServerNotifForwarder.broadcasterClass));
/*     */             }
/*     */           })).booleanValue();
/*     */       if (!bool)
...
/*     */       try {
/*     */         objectName = ObjectName.getInstance(this.mbeanServer.getDefaultDomain(), name.getKeyPropertyList());
...
/*     */     synchronized (this.listenerMap) {
/*     */       Set set = this.listenerMap.get(paramObjectName);
...
/*     */         set.toArray((Object[])arrayOfIdAndFilter);
/*     */       } 
...
/*     */     synchronized (this.listenerMap) {
/*     */       Set<?> set = this.listenerMap.get(MBeanServerDelegate.DELEGATE_NAME);
...
/*     */       for (IdAndFilter idAndFilter : arrayList) {
...
/* 229 */     this.bufferFilter = new NotifForwarderBufferFilter();
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */     
/* 491 */     this.listenerMap = new HashMap<>();
/*     */ 
/*     */     
/* 494 */     this.terminated = false;
/* 495 */     this.terminationLock = new int[0]; this.mbeanServer = paramMBeanServer; this.notifBuffer = paramNotificationBuffer; this.connectionId = paramString; this.connectionTimeout = EnvHelp.getServerConnectionTimeout(paramMap); String str = (String)paramMap.get("jmx.remote.x.check.notification.emission"); this.checkNotificationEmission = EnvHelp.computeBooleanFromString(str); this.notificationAccessController = EnvHelp.getNotificationAccessController(paramMap);
/*     */   } private void checkState() throws IOException { synchronized (this.terminationLock) { if (this.terminated)
/* 497 */         throw new IOException("The connection has been terminated.");  }  } static final String broadcasterClass = NotificationBroadcaster.class
/* 498 */     .getName(); private Integer getListenerID() {
/*     */     synchronized (listenerCounterLock) {
/*     */       return Integer.valueOf(listenerCounter++);
/*     */     } 
/*     */   } public final void checkMBeanPermission(ObjectName paramObjectName, String paramString) throws InstanceNotFoundException, SecurityException {
/*     */     checkMBeanPermission(this.mbeanServer, paramObjectName, paramString);
/* 504 */   } private static final ClassLogger logger = new ClassLogger("javax.management.remote.misc", "ServerNotifForwarder");
/*     */   
...
/*     */       try {
/*     */         objectInstance = AccessController.<ObjectInstance>doPrivileged(new PrivilegedExceptionAction<ObjectInstance>() {
...
/*     */       MBeanPermission mBeanPermission = new MBeanPermission(str, null, name, paramString);
...
/*     */       return (param1Object instanceof IdAndFilter && ((IdAndFilter)param1Object).getId().equals(getId()));
/*     */     }
/*     */   }
...
/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/jmx/remote/internal/ServerNotifForwarder.class
