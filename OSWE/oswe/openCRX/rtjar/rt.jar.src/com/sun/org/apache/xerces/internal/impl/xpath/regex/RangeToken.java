...
/*  37 */   int[] map = null; int nonMapIndex;
...
/*     */ 
/*     */           
/* 166 */           target += 2; continue;
/* 167 */         }  if (baseend < this.ranges[target + 1]) {
/* 168 */           if (DEBUG) {
...
/* 215 */         result[k++] = tok.ranges[j++]; continue;
/* 216 */       }  if (j >= tok.ranges.length) {
/* 217 */         result[k++] = this.ranges[i++];
/* 218 */         result[k++] = this.ranges[i++]; continue;
/* 219 */       }  if (tok.ranges[j] < this.ranges[i] || (tok.ranges[j] == this.ranges[i] && tok.ranges[j + 1] < this.ranges[i + 1])) {
/*     */         
/* 221 */         result[k++] = tok.ranges[j++];
/* 222 */         result[k++] = tok.ranges[j++]; continue;
/*     */       } 
...
/* 260 */         result[wp++] = this.ranges[src++]; continue;
/* 261 */       }  if (srcend >= subbegin && srcbegin <= subend) {
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */         
/* 346 */         src1 += 2; continue;
/* 347 */       }  if (src1end >= src2begin && src1begin <= src2end) {
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/* 386 */         this.ranges[src1] = src2end + 1; continue;
/*     */       } 
/* 388 */       if (src2end < src1begin) {
/*     */ 
/*     */ 
/*     */         
/* 392 */         src2 += 2; continue;
/*     */       } 
/* 394 */       throw new RuntimeException("Token#intersectRanges(): Internal Error: [" + this.ranges[src1] + "," + this.ranges[src1 + 1] + "] & [" + tok.ranges[src2] + "," + tok.ranges[src2 + 1] + "]");
/*     */     } 
/*     */ 
/*     */ 
/*     */ 
...
/* 462 */     for (int j = 0; j < uppers.ranges.length; j += 2) {
/* 463 */       for (int ch = uppers.ranges[j]; ch <= uppers.ranges[j + 1]; ch++) {
/* 464 */         if (ch > 65535) {
...
/* 517 */     int nonMapIndex = this.ranges.length; int i;
/* 518 */     for (i = 0; i < asize; i++) {
/* 519 */       map[i] = 0;
/*     */     }
/* 521 */     for (i = 0; i < this.ranges.length; i += 2) {
/* 522 */       int s = this.ranges[i];
/* 523 */       int e = this.ranges[i + 1];
/* 524 */       if (s < 256) {
/* 525 */         for (int j = s; j <= e && j < 256; j++) {
/* 526 */           map[j / 32] = map[j / 32] | 1 << (j & 0x1F);
/*     */         }
/*     */       } else {
/*     */         
/* 530 */         nonMapIndex = i;
...
/*     */       }
/*     */     
/* 571 */     } else if (this == Token.token_not_0to9) {
/* 572 */       ret = "\\D";
...
/*     */     String ret;
/* 599 */     switch (ch) { case 44: case 45: case 91: case 92:
/*     */       case 93:
/*     */       case 94:
/* 602 */         ret = "\\" + (char)ch;
/*     */ 
