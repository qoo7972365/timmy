...
/*      */     
/*  285 */     setChunkValue(this.fNodeValue, (Object)null, echunk, eindex);
/*      */     
/*  287 */     setChunkValue(this.fNodeURI, (Object)null, echunk, eindex);
/*      */ 
/*      */     
...
/*  394 */     return createDeferredElement((String)null, elementName);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  512 */     return createDeferredAttribute(attrName, (String)null, attrValue, specified);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */       
/*  696 */       int i = getChunkIndex(this.fNodePrevSib, oachunk, oaindex);
/*  697 */       if (nextIndex == -1) {
/*  698 */         setChunkIndex(this.fNodeExtra, i, echunk, eindex);
/*      */       } else {
/*      */         
/*  701 */         int pchunk = nextIndex >> 8;
/*  702 */         int pindex = nextIndex & 0xFF;
/*  703 */         setChunkIndex(this.fNodePrevSib, i, pchunk, pindex);
/*      */       } 
/*      */ 
/*      */       
...
/*      */       
/*  713 */       int attrTextIndex = clearChunkIndex(this.fNodeLastChild, oachunk, oaindex);
...
/*  804 */     return free ? clearChunkIndex(this.fNodeParent, chunk, index) : 
/*  805 */       getChunkIndex(this.fNodeParent, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/*  826 */     return free ? clearChunkIndex(this.fNodeLastChild, chunk, index) : 
/*  827 */       getChunkIndex(this.fNodeLastChild, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  892 */     return free ? clearChunkIndex(this.fNodePrevSib, chunk, index) : 
/*  893 */       getChunkIndex(this.fNodePrevSib, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  908 */       int nchunk = 0;
/*  909 */       int nindex = 0;
/*  910 */       int index = getChunkIndex(this.fNodeLastChild, nchunk, nindex);
/*  911 */       for (; index != -1; 
...
/*  928 */       index = getChunkIndex(this.fNodeLastChild, nchunk, nindex);
/*  929 */       for (; index != -1; 
/*  930 */         index = getChunkIndex(this.fNodePrevSib, nchunk, nindex)) {
/*      */         
/*  932 */         nchunk = index >> 8;
/*  933 */         nindex = index & 0xFF;
/*  934 */         if (getChunkIndex(this.fNodeType, nchunk, nindex) == 21 && 
/*      */           
/*  936 */           getChunkValue(this.fNodeName, nchunk, nindex) == elementName) {
/*  937 */           return index;
/*      */         }
/*      */       } 
/*      */     } 
...
/*  972 */           node = new DeferredAttrNSImpl(this, nodeIndex); break;
/*      */         } 
...
/* 1127 */     return free ? clearChunkValue(this.fNodeName, chunk, index) : 
/* 1128 */       getChunkValue(this.fNodeName, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/*      */     
/* 1150 */     String value = free ? clearChunkValue(this.fNodeValue, chunk, index) : getChunkValue(this.fNodeValue, chunk, index);
/* 1151 */     if (value == null) {
/* 1152 */       return null;
/*      */     }
/*      */     
...
/* 1158 */       if (prevSib != -1 && 
/* 1159 */         getNodeType(prevSib, false) == 3)
/*      */       {
/*      */ 
/*      */         
...
/*      */             break;
/*      */           }
/* 1175 */         } while (getNodeType(prevSib, false) == 3);
/*      */         
/* 1177 */         int chunkCount = this.fStrChunks.size();
/*      */ 
/*      */         
/* 1180 */         for (int i = chunkCount - 1; i >= 0; i--) {
/* 1181 */           this.fBufferStr.append(this.fStrChunks.get(i));
/*      */         }
/*      */         
...
/* 1207 */           this.fBufferStr.append(this.fStrChunks.get(i));
/*      */         }
/*      */         
...
/* 1266 */     return free ? clearChunkValue(this.fNodeValue, chunk, index) : 
/* 1267 */       getChunkValue(this.fNodeValue, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 1291 */     return free ? clearChunkIndex(this.fNodeExtra, chunk, index) : 
/* 1292 */       getChunkIndex(this.fNodeExtra, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/* 1313 */     return free ? (short)clearChunkIndex(this.fNodeType, chunk, index) : 
/* 1314 */       (short)getChunkIndex(this.fNodeType, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/* 1354 */     return free ? clearChunkValue(this.fNodeURI, chunk, index) : 
/* 1355 */       getChunkValue(this.fNodeURI, chunk, index);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */                 
/* 1575 */                 int nodeIndex = ((DeferredNode)child).getNodeIndex();
...
/* 1701 */       ChildNode lastNode = null; int index;
/* 1702 */       for (index = last; index != -1; 
...
/* 1751 */     int index = getLastChild(nodeIndex);
/* 1752 */     for (; index != -1; 
...
/* 1965 */     if (ovalue == -1) {
/* 1966 */       dataChunk[256] = dataChunk[256] + 1;
/*      */     }
...
/* 2024 */     if (value != -1) {
/* 2025 */       data[chunk][256] = data[chunk][256] - 1;
...
/* 2062 */       this.identifiers = new HashMap<>();
/*      */     }
/*      */ 
/*      */     
...
/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/org/apache/xerces/internal/dom/DeferredDocumentImpl.class
