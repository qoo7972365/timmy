...
/*      */ 
/*      */   
/*  146 */   private static final String[] RECOGNIZED_PROPERTIES = new String[] { "http://apache.org/xml/properties/dom/document-class-name", "http://apache.org/xml/properties/dom/current-element-node" };
/*      */ 
/*      */   
...
/*      */     }
/*      */     
/*  345 */     if (!documentClassName.equals("com.sun.org.apache.xerces.internal.dom.DocumentImpl") && 
/*  346 */       !documentClassName.equals("com.sun.org.apache.xerces.internal.dom.PSVIDocumentImpl")) {
/*      */       
/*      */       try {
...
/*  353 */               DOMMessageFormatter.formatMessage("http://www.w3.org/dom/DOMTR", "InvalidDocumentClassName", new Object[] { documentClassName }));
/*      */         
/*      */         }
/*      */       
/*      */       }
...
/*  360 */             DOMMessageFormatter.formatMessage("http://www.w3.org/dom/DOMTR", "MissingDocumentClassName", new Object[] { documentClassName }));
/*      */       } 
/*      */     }
/*      */ 
/*      */ 
...
/*      */     
/*  411 */     this
/*  412 */       .fCreateEntityRefNodes = this.fConfiguration.getFeature("http://apache.org/xml/features/dom/create-entity-ref-nodes");
/*      */     
/*  414 */     this
/*  415 */       .fIncludeIgnorableWhitespace = this.fConfiguration.getFeature("http://apache.org/xml/features/dom/include-ignorable-whitespace");
/*      */     
/*  417 */     this
/*  418 */       .fDeferNodeExpansion = this.fConfiguration.getFeature("http://apache.org/xml/features/dom/defer-node-expansion");
/*      */     
...
/*      */ 
/*      */     
/*  427 */     setDocumentClassName((String)this.fConfiguration
/*  428 */         .getProperty("http://apache.org/xml/properties/dom/document-class-name"));
/*      */ 
/*      */     
...
/*      */     else {
/*      */       
/*  537 */       int er = this.fDeferredDocumentImpl.createDeferredEntityReference(name, identifier.getExpandedSystemId());
...
/*  543 */           if (nodeType == 6) {
/*      */             
/*  545 */             String nodeName = this.fDeferredDocumentImpl.getNodeName(node, false);
...
/*  623 */       if (this.fDOMFilter != null && !this.fInEntityRef && (this.fDOMFilter
/*  624 */         .getWhatToShow() & 0x80) != 0) {
...
/*      */       } 
/*      */     } else {
/*  654 */       int comment = this.fDeferredDocumentImpl.createDeferredComment(text.toString());
...
/*      */       }
/*      */       
/*  700 */       ProcessingInstruction pi = this.fDocument.createProcessingInstruction(target, data.toString());
/*      */ 
/*      */       
/*  703 */       setCharacterData(false);
/*  704 */       this.fCurrentNode.appendChild(pi);
/*  705 */       if (this.fDOMFilter != null && !this.fInEntityRef && (this.fDOMFilter
/*  706 */         .getWhatToShow() & 0x40) != 0) {
...
/*      */       } 
/*      */     } else {
/*  730 */       int pi = this.fDeferredDocumentImpl.createDeferredProcessingInstruction(target, data.toString());
...
/*      */ 
/*      */           
/*  797 */           Class<?> defaultDocClass = ObjectFactory.findProviderClass("com.sun.org.apache.xerces.internal.dom.CoreDocumentImpl", true);
...
/*  823 */               DOMMessageFormatter.formatMessage("http://www.w3.org/dom/DOMTR", "CannotCreateDocumentClass", new Object[] { this.fDocumentClassName }));
/*      */         } 
/*      */       } 
/*      */ 
/*      */ 
...
/*      */       
/*  906 */       this
/*  907 */         .fDocumentTypeIndex = this.fDeferredDocumentImpl.createDeferredDocumentType(rootElement, publicId, systemId);
...
/*  931 */         this.fRejectedElementDepth++;
/*      */         return;
/*      */       } 
...
/*      */ 
/*      */         
/* 1110 */         this.fDeferredDocumentImpl.setDeferredAttribute(el, attributes
/*      */             
/* 1112 */             .getQName(i), attributes
/* 1113 */             .getURI(i), attributes
/* 1114 */             .getValue(i), attributes
...
/* 1164 */           this
/* 1165 */             .fCurrentCDATASection = this.fDocument.createCDATASection(text.toString());
...
/* 1188 */               ((Text)child).setNodeValue((String)null);
/*      */             } 
...
/*      */         
/* 1210 */         int cs = this.fDeferredDocumentImpl.createDeferredCDATASection(text.toString());
/*      */         
...
/*      */       else {
/*      */         
/* 1218 */         int txt = this.fDeferredDocumentImpl.createDeferredTextNode(text.toString(), false);
...
/*      */       
/* 1230 */       int txt = this.fDeferredDocumentImpl.createDeferredTextNode(value, false);
...
/*      */     else {
/*      */       
/* 1274 */       int txt = this.fDeferredDocumentImpl.createDeferredTextNode(text.toString(), true);
...
/* 1328 */           Node parent; NodeList ls; int length, i; short code = this.fDOMFilter.acceptNode(this.fCurrentNode);
/* 1329 */           switch (code) {
/*      */             case 4:
/* 1331 */               throw Abort.INSTANCE;
/*      */             
/*      */             case 2:
/* 1334 */               parent = this.fCurrentNode.getParentNode();
...
/*      */ 
/*      */               
/* 1348 */               parent = this.fCurrentNode.getParentNode();
/* 1349 */               ls = this.fCurrentNode.getChildNodes();
/* 1350 */               length = ls.getLength();
/*      */               
...
/*      */       } 
/* 1387 */       this
/* 1388 */         .fCurrentNodeIndex = this.fDeferredDocumentImpl.getParentNode(this.fCurrentNodeIndex, false);
/*      */     } 
/*      */   }
/*      */ 
/*      */ 
...
/*      */       {
/* 1431 */         if (this.fDOMFilter != null && !this.fInEntityRef && (this.fDOMFilter
/* 1432 */           .getWhatToShow() & 0x8) != 0) {
/* 1433 */           Node parent; short code = this.fDOMFilter.acceptNode(this.fCurrentCDATASection);
/* 1434 */           switch (code) {
/*      */             case 4:
...
/*      */             case 2:
/*      */             case 3:
/* 1442 */               parent = this.fCurrentNode.getParentNode();
...
/* 1460 */       this
/* 1461 */         .fCurrentNodeIndex = this.fDeferredDocumentImpl.getParentNode(this.fCurrentNodeIndex, false);
...
/*      */         }
/*      */         
/* 1552 */         if (this.fDOMFilter != null && (this.fDOMFilter
/* 1553 */           .getWhatToShow() & 0x10) != 0) {
/* 1554 */           Node parent; short code = this.fDOMFilter.acceptNode(this.fCurrentNode);
/* 1555 */           switch (code) {
/*      */             case 4:
/* 1557 */               throw Abort.INSTANCE;
/*      */             
/*      */             case 2:
/* 1560 */               parent = this.fCurrentNode.getParentNode();
...
/* 1594 */           if (node != null && node.getNodeType() == 3 && child
/* 1595 */             .getNodeType() == 3) {
...
/* 1620 */           if (nodeType == 6) {
/*      */             
/* 1622 */             String nodeName = this.fDeferredDocumentImpl.getNodeName(node, false);
...
/*      */       } 
/*      */       
/* 1632 */       if (this.fDeferredEntityDecl != -1 && this.fDeferredDocumentImpl
/* 1633 */         .getLastChild(this.fDeferredEntityDecl, false) == -1) {
/*      */         
...
/* 1645 */         this
/* 1646 */           .fCurrentNodeIndex = this.fDeferredDocumentImpl.getParentNode(this.fCurrentNodeIndex, false);
/*      */ 
/*      */       
/*      */       }
/*      */       else {
...
/*      */         
/* 1655 */         int parentIndex = this.fDeferredDocumentImpl.getParentNode(this.fCurrentNodeIndex, false);
/*      */ 
/*      */         
...
/*      */     }
/*      */     
/* 1832 */     String internalSubset = (this.fInternalSubset != null && this.fInternalSubset.length() > 0) ? this.fInternalSubset.toString() : null;
...
/* 1946 */       this.fInternalSubset.append(singleQuote ? 39 : 34);
/* 1947 */       this.fInternalSubset.append(value);
/* 1948 */       this.fInternalSubset.append(singleQuote ? 39 : 34);
...
/* 1965 */         entity.setBaseURI(this.fBaseURIStack.peek());
...
/* 1985 */       if (!found) {
/*      */         
/* 1987 */         int entityIndex = this.fDeferredDocumentImpl.createDeferredEntity(name, (String)null, (String)null, (String)null, this.fBaseURIStack.peek());
...
/*      */       } 
/* 2078 */       if (!found) {
/* 2079 */         int entityIndex = this.fDeferredDocumentImpl.createDeferredEntity(name, publicId, literalSystemId, (String)null, identifier
/* 2080 */             .getBaseSystemId());
...
/* 2114 */     if (augs != null && this.fInternalSubset != null && !this.fInDTDExternalSubset && Boolean.TRUE
/*      */       
/* 2116 */       .equals(augs.getItem("ENTITY_SKIPPED"))) {
...
/*      */       } 
/* 2221 */       if (!found) {
/* 2222 */         int entityIndex = this.fDeferredDocumentImpl.createDeferredEntity(name, publicId, literalSystemId, notation, identifier
/* 2223 */             .getBaseSystemId());
...
/*      */       } 
/* 2296 */       if (!found) {
/* 2297 */         int notationIndex = this.fDeferredDocumentImpl.createDeferredNotation(name, publicId, literalSystemId, identifier
/* 2298 */             .getBaseSystemId());
...
/*      */ 
/*      */           
/* 2436 */           if (attributeName.startsWith("xmlns:") || attributeName
/* 2437 */             .equals("xmlns")) {
...
/*      */           } 
/*      */         }
/* 2444 */         int attrIndex = this.fDeferredDocumentImpl.createDeferredAttribute(attributeName, namespaceURI, defaultValue
/* 2445 */             .toString(), false);
...
/*      */ 
/*      */           
/* 2480 */           if (attributeName.startsWith("xmlns:") || attributeName
/* 2481 */             .equals("xmlns")) {
...
/* 2624 */         child.getNodeType() == 3 && (this.fDOMFilter
/* 2625 */         .getWhatToShow() & 0x4) != 0) {
...
/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/org/apache/xerces/internal/parsers/AbstractDOMParser.class
