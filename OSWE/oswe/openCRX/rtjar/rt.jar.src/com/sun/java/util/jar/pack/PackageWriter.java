...
/* 1710 */     this.codeHist = new int[256];
/* 1711 */     this.ldcHist = new int[20]; this.pkg = paramPackage;
/*      */     this.finalOut = paramOutputStream;
/* 1713 */     initHighestClassVersion(paramPackage.getHighestClassVersion()); } void printCodeHist() { assert this.verbose > 0;
/* 1714 */     String[] arrayOfString = new String[this.codeHist.length];
/* 1715 */     int i = 0; int j;
/* 1716 */     for (j = 0; j < this.codeHist.length; j++) {
/* 1717 */       i += this.codeHist[j];
/*      */     }
/* 1719 */     for (j = 0; j < this.codeHist.length; j++) {
/* 1720 */       if (this.codeHist[j] == 0) { arrayOfString[j] = ""; }
/* 1721 */       else { String str1 = Instruction.byteName(j);
/* 1722 */         String str2 = "" + this.codeHist[j];
/* 1723 */         str2 = "         ".substring(str2.length()) + str2;
/* 1724 */         String str3 = "" + (this.codeHist[j] * 10000 / i);
/* 1725 */         while (str3.length() < 4) {
/* 1726 */           str3 = "0" + str3;
/*      */         }
/* 1728 */         str3 = str3.substring(0, str3.length() - 2) + "." + str3.substring(str3.length() - 2);
/* 1729 */         arrayOfString[j] = str2 + "  " + str3 + "%  " + str1; }
/*      */     
/* 1731 */     }  Arrays.sort((Object[])arrayOfString);
/* 1732 */     System.out.println("Bytecode histogram [" + i + "]");
/* 1733 */     for (j = arrayOfString.length; --j >= 0; ) {
/* 1734 */       if ("".equals(arrayOfString[j]))
/* 1735 */         continue;  System.out.println(arrayOfString[j]);
/*      */     } 
/* 1737 */     for (j = 0; j < this.ldcHist.length; j++) {
/* 1738 */       int k = this.ldcHist[j];
/* 1739 */       if (k != 0)
/* 1740 */         System.out.println("ldc " + ConstantPool.tagName(j) + " " + k); 
/*      */     }  }
/*      */ 
/*      */   
...
/*      */       if (testBit(this.archiveOptions, 8))
...
/*      */     if (!bool1) {
/*      */       i = bool1 | ((this.band_headers.length() != 0) ? 1 : 0);
/*      */       i |= (this.attrDefsWritten.length != 0) ? 1 : 0;
/*      */       if (i != 0)
...
/*      */     if (!bool2) {
/*      */       j = bool2 | ((this.archiveNextCount > 0) ? 1 : 0);
/*      */       j |= (this.pkg.default_modtime != 0) ? 1 : 0;
/*      */       if (j != 0)
...
/*      */     if (j != 0) {
/*      */       int k = (int)(this.archiveSize1 >>> 32L);
/*      */       int m = (int)(this.archiveSize1 >>> 0L);
/*      */       this.archive_header_S.patchValue(0, k);
/*      */       this.archive_header_S.patchValue(1, m);
/*      */       int n = UNSIGNED5.getLength(0);
/*      */       this.archiveSize0 += (UNSIGNED5.getLength(k) - n);
/*      */       this.archiveSize0 += (UNSIGNED5.getLength(m) - n);
/*      */     } 
/*      */     if (this.verbose > 1)
...
/*      */       switch (b) {
/*      */         case 1:
/*      */           if (i > 0 && !$assertionsDisabled && this.pkg.cp.getIndexByTag(b).get(0) != ConstantPool.getUtf8Entry(""))
...
/*      */     return this.pkg.cp.getIndexByTag(paramByte);
/*      */   }
/*      */   
...
/*      */             this.cp_Long_hi.putInt((int)(l >>> 32L));
/*      */             this.cp_Long_lo.putInt((int)(l >>> 0L));
/*      */           } 
/*      */           break;
/*      */         case 6:
/*      */           for (b1 = 0; b1 < arrayOfEntry.length; b1++) {
...
/*      */             this.cp_Double_hi.putInt((int)(l >>> 32L));
/*      */             this.cp_Double_lo.putInt((int)(l >>> 0L));
/*      */           } 
/*      */           break;
/*      */         case 8:
/*      */           for (b1 = 0; b1 < arrayOfEntry.length; b1++) {
...
/*      */     for (byte b = 0; b < i; b++)
/*      */       arrayOfInt1[b] = paramArrayOfchar[paramInt3 + b]; 
...
/*      */       int k = arrayOfInt[1];
/*      */       int m = Math.max(5, k / 1000);
/*      */       if (this.verbose > 1)
/*      */         Utils.log.fine("big string[" + paramInt1 + "] normalSize=" + arrayOfInt[0] + "/z=" + arrayOfInt[1] + " win=" + ((j < k - m) ? 1 : 0)); 
/*      */       if (j < k - m) {
...
/*      */       if (bool1)
/*      */         this.file_size_hi.putInt((int)(l >>> 32L)); 
/*      */       if (bool2)
...
/*      */     byte b;
/*      */     for (b = 0; b < 4; b++)
/*      */       this.allLayouts.set(b, (Map)new HashMap<>()); 
/*      */     for (Package.Class clazz : this.pkg.classes) {
...
/*      */     this.backCountTable = (Map)new HashMap<>();
...
/*      */       byte b1 = 0;
/*      */       Map map = this.allLayouts.get(b);
/*      */       Map.Entry[] arrayOfEntry = new Map.Entry[map.size()];
/*      */       map.entrySet().toArray((Object[])arrayOfEntry);
/*      */       Arrays.sort(arrayOfEntry, (Comparator)new Comparator<Map.Entry<Attribute.Layout, int[]>>() {
...
/*      */               if (i != 0)
/*      */                 return i; 
/*      */               return ((Attribute.Layout)param1Entry1.getKey()).compareTo(param1Entry2.getKey());
/*      */             }
/*      */           });
...
/*      */         Integer integer = this.attrIndexTable.get(layout);
/*      */         if (integer != null) {
/*      */           j = integer.intValue();
/*      */         } else if (l != 0L) {
/*      */           while ((l & 0x1L) == 0L) {
...
/*      */       Map<Attribute.Layout, int[]> map = this.allLayouts.get(paramInt);
...
/*      */             continue; 
/*      */         } 
/*      */         Attribute.Layout layout = ((List<Attribute.Layout>)this.attrDefs.get(i)).get(b);
/*      */         arrayList.add(new Object[] { Integer.valueOf(k), layout });
...
/*      */     Arrays.sort(arrayOfObject, (Comparator)new Comparator<Object[]>() {
/*      */           public int compare(Object[] param1ArrayOfObject1, Object[] param1ArrayOfObject2) {
/*      */             int i = ((Comparable<Object>)param1ArrayOfObject1[0]).compareTo(param1ArrayOfObject2[0]);
/*      */             if (i != 0)
/*      */               return i; 
/*      */             Integer integer1 = PackageWriter.this.attrIndexTable.get(param1ArrayOfObject1[1]);
/*      */             Integer integer2 = PackageWriter.this.attrIndexTable.get(param1ArrayOfObject2[1]);
...
/*      */       int[] arrayOfInt = Arrays.copyOf(this.attrIndexLimit, 4);
...
/*      */       ((List)this.attrDefs.get(b)).toArray((Object[])arrayOfLayout);
/*      */       boolean bool;
...
/*      */             if (i != 0) {
/*      */               int[] arrayOfInt = this.backCountTable.get(layout);
...
/*      */     hashMap.values().toArray((Object[])arrayOfInnerClass);
/*      */     hashMap = null;
/*      */     Arrays.sort((Object[])arrayOfInnerClass);
...
/*      */     List<Package.InnerClass> list = paramClass.getInnerClasses();
...
/*      */       b1++;
/*      */       if (this.verbose > 0 && b1 % 1000 == 0)
/*      */         Utils.log.info("Have scanned " + b1 + " classes..."); 
/*      */     } 
/*      */   }
/*      */   
/*      */   void writeMembers(Package.Class paramClass) throws IOException {
/*      */     List<Package.Class.Field> list = paramClass.getFields();
/*      */     this.class_field_count.putInt(list.size());
/*      */     for (Package.Class.Field field : list) {
/*      */       this.field_descr.putRef(field.getDescriptor());
/*      */       writeAttrs(1, field, paramClass);
/*      */     } 
/*      */     List<Package.Class.Method> list1 = paramClass.getMethods();
/*      */     this.class_method_count.putInt(list1.size());
/*      */     for (Package.Class.Method method : list1) {
...
/*      */     byte b;
/*      */     int i;
/*      */     for (b = 0, i = paramCode.getHandlerCount(); b < i; b++) {
...
/*      */     long l2 = 0L;
/*      */     byte b = 0;
/*      */     for (Attribute attribute : h.attributes) {
/*      */       Attribute.Layout layout = attribute.layout();
/*      */       int i = ((Integer)this.attrIndexTable.get(layout)).intValue();
/*      */       assert ((List<Attribute.Layout>)this.attrDefs.get(paramInt)).get(i) == layout;
/*      */       if (this.verbose > 3)
/*      */         Utils.log.fine("add attr @" + i + " " + attribute + " in " + h); 
/*      */       if (i < this.attrIndexLimit[paramInt] && testBit(l1, 1L << i)) {
/*      */         if (this.verbose > 3)
/*      */           Utils.log.fine("Adding flag bit 1<<" + i + " in " + Long.toHexString(l1)); 
/*      */         assert !testBit(h.flags, 1L << i);
/*      */         l2 |= 1L << i;
/*      */         l1 -= 1L << i;
/*      */       } else {
/*      */         l2 |= 0x10000L;
...
/*      */       final BandStructure.Band[] ab = this.attrBandTable.get(layout);
/*      */       assert arrayOfBand != null;
/*      */       assert arrayOfBand.length == layout.bandCount;
/*      */       final int[] bc = this.backCountTable.get(layout);
/*      */       assert arrayOfInt != null;
/*      */       assert arrayOfInt.length == (layout.getCallables()).length;
/*      */       if (this.verbose > 2)
/*      */         Utils.log.fine("writing " + attribute + " in " + h); 
/*      */       boolean bool1 = (paramInt == 1 && layout == this.attrConstantValue) ? true : false;
/*      */       if (bool1)
/*      */         setConstantValueIndex((Package.Class.Field)h); 
...
/*      */               ((BandStructure.IntBand)ab[param1Int1]).putInt(param1Int2);
/*      */             }
/*      */             
/*      */             public void putRef(int param1Int, ConstantPool.Entry param1Entry) {
/*      */               ((BandStructure.CPRefBand)ab[param1Int]).putRef(param1Entry);
/*      */             }
/*      */             
...
/*      */       intBand.putInt(b);
/*      */     } 
/*      */     intBand2.putInt(h.flags | (int)l2);
/*      */     if (bool) {
/*      */       intBand1.putInt((int)(l2 >>> 32L));
/*      */     } else {
/*      */       assert l2 >>> 32L == 0L;
/*      */     } 
/*      */     assert (h.flags & l2) == 0L : h + ".flags=" + Integer.toHexString(h.flags) + "^" + Long.toHexString(l2);
/*      */   }
/*      */   
...
/*      */           boolean bool2 = (k >= 216) ? true : false;
/*      */           boolean bool3 = bool;
/*      */           bool = false;
/*      */           if (bool3)
/*      */             k += 7; 
...
/*      */               switch_ = (Instruction.Switch)instruction;
/*      */               n = switch_.getAlignedPC();
/*      */               i1 = switch_.getNextPC();
/*      */               i2 = switch_.getCaseCount();
/*      */               this.bc_case_count.putInt(i2);
/*      */               putLabel(this.bc_label, paramCode, instruction.getPC(), switch_.getDefaultLabel());
/*      */               for (i3 = 0; i3 < i2; i3++)
/*      */                 putLabel(this.bc_label, paramCode, instruction.getPC(), switch_.getCaseLabel(i3)); 
/*      */               if (i == 170) {
/*      */                 this.bc_case_value.putInt(switch_.getCaseValue(0));
/*      */                 break;
/*      */               } 
/*      */               for (i3 = 0; i3 < i2; i3++)
/*      */                 this.bc_case_value.putInt(switch_.getCaseValue(i3)); 
/*      */               break;
/*      */             default:
...
/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/java/util/jar/pack/PackageWriter.class
