...
/* 167 */       for (int j = this.prefixes.length; j < buf.length; j++)
/* 168 */         buf[j] = new Encoded(); 
...
/*     */     
/* 205 */     if (p.length() == 0) {
/* 206 */       if (this.nsContext.getCurrent().isRootElement() && this.nsContext
/* 207 */         .getNamespaceURI(prefixIndex).length() == 0)
/*     */         return; 
...
/* 416 */     this.XMLNS_EQUALS = (byte[])_XMLNS_EQUALS.clone();
/* 417 */     this.XMLNS_COLON = (byte[])_XMLNS_COLON.clone();
/* 418 */     this.EQUALS = (byte[])_EQUALS.clone();
/* 419 */     this.CLOSE_TAG = (byte[])_CLOSE_TAG.clone();
/* 420 */     this.EMPTY_TAG = (byte[])_EMPTY_TAG.clone();
/* 421 */     this.XML_DECL = (byte[])_XML_DECL.clone(); this.out = out; this.localNames = localNames;
/*     */     for (int i = 0; i < this.prefixes.length; i++)
/*     */       this.prefixes[i] = new Encoded(); 
/* 424 */     this.escapeHandler = escapeHandler; } private static final byte[] _XMLNS_EQUALS = toBytes(" xmlns=\"");
...
/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/xml/internal/bind/v2/runtime/output/UTF8XmlOutput.class
