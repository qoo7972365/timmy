...
/*      */ {
/*   76 */   private static final char[] XML_NAMESPACE_NAME_CHARS = "http://www.w3.org/XML/1998/namespace"
/*   77 */     .toCharArray();
/*      */   
/*   79 */   private static final char[] XMLNS_NAMESPACE_PREFIX_CHARS = "xmlns"
/*   80 */     .toCharArray();
/*      */   
/*   82 */   private static final char[] XMLNS_NAMESPACE_NAME_CHARS = "http://www.w3.org/2000/xmlns/"
/*   83 */     .toCharArray();
/*      */ 
/*      */ 
/*      */   
...
/*      */   
/*      */   static {
/*  102 */     String p = System.getProperty("com.sun.xml.internal.fastinfoset.parser.string-interning", 
/*  103 */         Boolean.toString(_stringInterningSystemDefault));
/*  104 */     _stringInterningSystemDefault = Boolean.valueOf(p).booleanValue();
/*      */     
/*  106 */     p = System.getProperty("com.sun.xml.internal.fastinfoset.parser.buffer-size", 
...
/*      */ 
/*      */   
/*  269 */   protected DuplicateAttributeVerifier _duplicateAttributeVerifier = new DuplicateAttributeVerifier(); protected static final int NISTRING_STRING = 0;
...
/*      */   
/*      */   public void reset() {
/*  388 */     this._terminate = this._doubleTerminate = false;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  434 */       decodeNonEmptyOctetStringOnSecondBitAsUtf8String();
/*      */       
...
/*  511 */           CommonResourceBundle.getInstance().getString("message.noExternalVocabularies"));
/*      */     }
/*      */ 
/*      */     
/*  515 */     String externalVocabularyURI = decodeNonEmptyOctetStringOnSecondBitAsUtf8String();
/*      */     
...
/*  531 */           .getString("message.externalVocabularyNotRegistered", new Object[] { externalVocabularyURI }));
/*      */     } 
/*      */   }
/*      */ 
/*      */   
...
/*      */ 
/*      */       
/*  638 */       String system_identifier = ((this._b & 0x2) > 0) ? decodeIdentifyingNonEmptyStringOnFirstBit(this._v.otherURI) : "";
/*      */       
/*  640 */       String public_identifier = ((this._b & 0x1) > 0) ? decodeIdentifyingNonEmptyStringOnFirstBit(this._v.otherURI) : "";
/*      */       
...
/*      */ 
/*      */       
/*  665 */       String public_identifier = ((this._b & 0x1) > 0) ? decodeIdentifyingNonEmptyStringOnFirstBit(this._v.otherURI) : "";
/*      */       
...
/*      */       case 0:
/*  686 */         data = new String(this._charBuffer, 0, this._charBufferLength);
/*  687 */         if (this._addToTable) {
/*  688 */           this._v.otherString.add((CharArray)new CharArrayString(data));
/*      */         }
/*  690 */         return data;
/*      */       case 2:
...
/*      */ 
/*      */             
/*  730 */             decodeIdentifyingNonEmptyStringOnFirstBit(this._v.localName), -1, -1, this._identifier, (char[])null);
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  740 */             decodeIdentifyingNonEmptyStringOnFirstBit(this._v.localName), -1, this._namespaceNameIndex, this._identifier, (char[])null);
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  753 */             decodeIdentifyingNonEmptyStringOnFirstBit(this._v.localName), this._prefixIndex, this._namespaceNameIndex, this._identifier, this._charBuffer);
/*      */     } 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */         
/*  791 */         length = read() << 24 | read() << 16 | read() << 8 | read();
...
/*      */ 
/*      */         
/*  812 */         length = read() << 24 | read() << 16 | read() << 8 | read();
...
/*      */       
/*      */       case 2:
/*  874 */         length = read() << 24 | read() << 16 | read() << 8 | read();
...
/*      */ 
/*      */           
/*  905 */           ._octetBufferLength = read() << 24 | read() << 16 | read() << 8 | read();
...
/*  924 */         s = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
...
/*  931 */         s = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
...
/*      */       
/*      */       case 2:
/*  940 */         length = read() << 24 | read() << 16 | read() << 8 | read();
/*  941 */         this._octetBufferLength = length + 321;
/*  942 */         str1 = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
/*  943 */         this._identifier = table.add(str1) - 1;
/*  944 */         return str1;
/*      */       
/*      */       case 3:
...
/*      */         }
/*      */         
/*  981 */         s = this._stringInterning ? (new String(this._charBuffer, 0, this._charBufferLength)).intern() : new String(this._charBuffer, 0, this._charBufferLength);
/*      */         
...
/*      */         }
/*      */         
/*  999 */         s = this._stringInterning ? (new String(this._charBuffer, 0, this._charBufferLength)).intern() : new String(this._charBuffer, 0, this._charBufferLength);
/*      */         
...
/* 1009 */         s = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
...
/* 1016 */         s = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
...
/*      */       
/*      */       case 2:
/* 1025 */         length = read() << 24 | read() << 16 | read() << 8 | read();
/* 1026 */         this._octetBufferLength = length + 321;
/* 1027 */         str1 = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
/* 1028 */         this._prefixIndex = this._v.prefix.add(str1);
/* 1029 */         return str1;
/*      */       
/*      */       case 10:
...
/* 1106 */         s = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
...
/*      */         }
/*      */         
/* 1119 */         s = this._stringInterning ? (new String(this._charBuffer, 0, this._charBufferLength)).intern() : new String(this._charBuffer, 0, this._charBufferLength);
/*      */         
...
/*      */         }
/*      */         
/* 1133 */         s = this._stringInterning ? (new String(this._charBuffer, 0, this._charBufferLength)).intern() : new String(this._charBuffer, 0, this._charBufferLength);
/*      */         
...
/* 1141 */         s = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
...
/*      */       
/*      */       case 2:
/* 1150 */         length = read() << 24 | read() << 16 | read() << 8 | read();
/* 1151 */         this._octetBufferLength = length + 321;
/* 1152 */         str1 = this._stringInterning ? decodeUtf8StringAsString().intern() : decodeUtf8StringAsString();
/* 1153 */         this._namespaceNameIndex = this._v.namespaceName.add(str1);
/* 1154 */         return str1;
/*      */       
/*      */       case 10:
...
/* 1210 */     while (--i >= 0) {
/* 1211 */       if (c[i] != this._charBuffer[i]) {
/* 1212 */         return false;
/*      */       }
/*      */     } 
/* 1215 */     return true;
...
/*      */       
/*      */       case 2:
/* 1251 */         length = read() << 24 | read() << 16 | read() << 8 | read();
...
/* 1293 */       decodeFourBitAlphabetOctetsAsCharBuffer(BuiltInRestrictedAlphabets.table[this._identifier]);
/*      */     }
...
/* 1366 */         if (octetPosition != this._octetBufferLength - 1) {
...
/* 1474 */         this._charBuffer[this._charBufferLength++] = (char)b1; continue;
/*      */       } 
...
/* 1488 */         ch[this._charBufferLength++] = (char)b1; continue;
/*      */       } 
/* 1490 */       decodeTwoToFourByteUtf8Character(ch, b1, end);
/*      */     } 
/*      */     
/* 1493 */     this._charBufferLength -= offset; } private void decodeTwoToFourByteUtf8Character(int b1, int end) throws IOException {
/*      */     int b2;
/*      */     char c;
/*      */     int supplemental;
...
/*      */         }
/* 1504 */         b2 = this._octetBuffer[this._octetBufferOffset++] & 0xFF;
/* 1505 */         if ((b2 & 0xC0) != 128) {
...
/*      */       
/*      */       case 3:
/* 1518 */         c = decodeUtf8ThreeByteChar(end, b1);
...
/*      */       
/*      */       case 4:
/* 1527 */         supplemental = decodeUtf8FourByteChar(end, b1);
...
/*      */         }
/* 1549 */         b2 = this._octetBuffer[this._octetBufferOffset++] & 0xFF;
/* 1550 */         if ((b2 & 0xC0) != 128) {
...
/*      */       
/*      */       case 3:
/* 1563 */         c = decodeUtf8ThreeByteChar(end, b1);
...
/*      */       
/*      */       case 4:
/* 1572 */         supplemental = decodeUtf8FourByteChar(end, b1);
...
/* 1604 */         this._charBuffer[this._charBufferLength++] = (char)b1; continue;
/*      */       } 
...
/*      */         }
/* 1619 */         b2 = this._octetBuffer[this._octetBufferOffset++] & 0xFF;
/* 1620 */         if ((b2 & 0xC0) != 128) {
/* 1621 */           decodeUtf8StringIllegalState();
/*      */         }
/*      */         
/* 1624 */         c1 = (char)((b1 & 0x1F) << 6 | b2 & 0x3F);
/*      */ 
/*      */         
/* 1627 */         if (XMLChar.isNCNameStart(c1)) {
/* 1628 */           this._charBuffer[this._charBufferLength++] = c1;
/*      */         } else {
...
/*      */       
/*      */       case 3:
/* 1635 */         c = decodeUtf8ThreeByteChar(end, b1);
...
/*      */       
/*      */       case 4:
/* 1644 */         supplemental = decodeUtf8FourByteChar(end, b1);
...
/*      */         }
/* 1668 */         b2 = this._octetBuffer[this._octetBufferOffset++] & 0xFF;
/* 1669 */         if ((b2 & 0xC0) != 128) {
/* 1670 */           decodeUtf8StringIllegalState();
/*      */         }
/*      */         
/* 1673 */         c1 = (char)((b1 & 0x1F) << 6 | b2 & 0x3F);
/*      */ 
/*      */         
/* 1676 */         if (XMLChar.isNCName(c1)) {
/* 1677 */           this._charBuffer[this._charBufferLength++] = c1;
/*      */         } else {
...
/*      */       
/*      */       case 3:
/* 1684 */         c = decodeUtf8ThreeByteChar(end, b1);
...
/*      */       
/*      */       case 4:
/* 1693 */         supplemental = decodeUtf8FourByteChar(end, b1);
...
/* 1995 */     int headerSize = 4;
/*      */     
/* 1997 */     byte[] header = new byte[4];
/* 1998 */     int readBytesCount = s.read(header);
/* 1999 */     if (readBytesCount < 4 || header[0] != EncodingConstants.BINARY_HEADER[0] || header[1] != EncodingConstants.BINARY_HEADER[1] || header[2] != EncodingConstants.BINARY_HEADER[2] || header[3] != EncodingConstants.BINARY_HEADER[3])
/*      */     {
/*      */ 
/*      */ 
/*      */       
...
/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/xml/internal/fastinfoset/Decoder.class
