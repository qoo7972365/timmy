...
/*  105 */   private static int[] DATE_TIME_CHARACTERS_TABLE = new int[maxCharacter("0123456789-:TZ ") + 1]; private boolean _ignoreDTD; private boolean _ignoreComments; static {
/*      */     int i;
/*  107 */     for (i = 0; i < NUMERIC_CHARACTERS_TABLE.length; i++) {
/*  108 */       NUMERIC_CHARACTERS_TABLE[i] = -1;
/*      */     }
/*  110 */     for (i = 0; i < DATE_TIME_CHARACTERS_TABLE.length; i++) {
/*  111 */       DATE_TIME_CHARACTERS_TABLE[i] = -1;
/*      */     }
/*      */     
/*  114 */     for (i = 0; i < "0123456789-+.E ".length(); i++) {
/*  115 */       NUMERIC_CHARACTERS_TABLE["0123456789-+.E ".charAt(i)] = i;
/*      */     }
/*  117 */     for (i = 0; i < "0123456789-:TZ ".length(); i++)
...
/*      */ 
/*      */       
/*  819 */       boolean canAddCharacterContentToTable = canAddCharacterContentToTable(length, this._v.characterContentChunk);
/*      */ 
/*      */ 
/*      */ 
/*      */       
/*  824 */       int index = canAddCharacterContentToTable ? this._v.characterContentChunk.obtainIndex(ch, offset, length, true) : this._v.characterContentChunk.get(ch, offset, length);
/*      */       
/*  826 */       if (index != -1) {
/*      */         
/*  828 */         this._b = 160;
/*  829 */         encodeNonZeroIntegerOnFourthBit(index); return;
/*      */       } 
...
/*      */ 
/*      */       
/*  869 */       boolean canAddCharacterContentToTable = canAddCharacterContentToTable(length, this._v.characterContentChunk);
/*      */ 
/*      */ 
/*      */ 
/*      */       
/*  874 */       int index = canAddCharacterContentToTable ? this._v.characterContentChunk.obtainIndex(ch, offset, length, true) : this._v.characterContentChunk.get(ch, offset, length);
/*      */       
/*  876 */       if (index != -1) {
/*      */         
/*  878 */         this._b = 160;
/*  879 */         encodeNonZeroIntegerOnFourthBit(index); return;
/*      */       } 
...
/* 1001 */         if ((prefix == (names[i]).prefix || prefix.equals((names[i]).prefix)) && (namespaceURI == (names[i]).namespaceName || namespaceURI
/* 1002 */           .equals((names[i]).namespaceName))) {
...
/* 1087 */         if ((prefix == (names[i]).prefix || prefix.equals((names[i]).prefix)) && (namespaceURI == (names[i]).namespaceName || namespaceURI
/* 1088 */           .equals((names[i]).namespaceName))) {
...
/* 1115 */         if (namespaceURI == "http://www.w3.org/2000/xmlns/" || namespaceURI
/* 1116 */           .equals("http://www.w3.org/2000/xmlns/")) {
/* 1117 */           return false;
/*      */         }
...
/*      */ 
/*      */       
/* 1186 */       boolean canAddAttributeToTable = (mustBeAddedToTable || canAddAttributeToTable(s.length()));
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1191 */       int index = canAddAttributeToTable ? map.obtainIndex(s) : map.get(s);
/*      */       
/* 1193 */       if (index != -1) {
/*      */         
...
/*      */ 
/*      */       
/* 1233 */       boolean canAddCharacterContentToTable = canAddCharacterContentToTable(length, map);
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1238 */       int index = canAddCharacterContentToTable ? map.obtainIndex(ch, 0, length, false) : map.get(ch, 0, length);
/*      */       
/* 1240 */       if (index != -1) {
/*      */         
...
/*      */ 
/*      */       
/* 1282 */       boolean canAddCharacterContentToTable = canAddCharacterContentToTable(length, map);
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1287 */       int index = canAddCharacterContentToTable ? map.obtainIndex(ch, offset, length, clone) : map.get(ch, offset, length);
/*      */       
/* 1289 */       if (index != -1) {
/*      */         
...
/*      */ 
/*      */       
/* 1339 */       boolean canAddAttributeToTable = (mustBeAddedToTable || canAddAttributeToTable(s.length()));
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1344 */       int index = canAddAttributeToTable ? this._v.attributeValue.obtainIndex(s) : this._v.attributeValue.get(s);
/*      */       
/* 1346 */       if (index != -1) {
/*      */         
/* 1348 */         encodeNonZeroIntegerOnSecondBitFirstBitOne(index); return;
/*      */       } 
...
/*      */ 
/*      */     
/* 1519 */     int octetLength = BuiltInEncodingAlgorithmFactory.getAlgorithm(id).getOctetLengthFromPrimitiveLength(length);
/*      */     
...
/* 1525 */       .encodeToBytes(data, offset, length, this._octetBuffer, this._octetBufferIndex);
...
/*      */ 
/*      */       
/* 1549 */       boolean canAddCharacterContentToTable = canAddCharacterContentToTable(length, map);
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1554 */       int index = canAddCharacterContentToTable ? map.obtainIndex(ch, offset, length, clone) : map.get(ch, offset, length);
/*      */       
/* 1556 */       if (index != -1) {
/*      */         
/* 1558 */         this._b = 160;
...
/*      */ 
/*      */     
/* 1748 */     int octetLength = BuiltInEncodingAlgorithmFactory.getAlgorithm(id).getOctetLengthFromPrimitiveLength(length);
/*      */     
...
/* 1754 */       .encodeToBytes(data, offset, length, this._octetBuffer, this._octetBufferIndex);
...
/* 2075 */           CommonResourceBundle.getInstance().getString("message.integerMaxSize", new Object[] {
/* 2076 */               Integer.valueOf(1048576)
/*      */             }));
/*      */     } 
/*      */   }
/*      */ 
...
/*      */   
/*      */   protected Encoder() {
/* 2365 */     this._encodingBufferOutputStream = new EncodingBufferOutputStream();
/*      */     
/* 2367 */     this._encodingBuffer = new byte[512]; setCharacterEncodingScheme(_characterEncodingSchemeSystemDefault); } protected Encoder(boolean useLocalNameAsKeyForQualifiedNameLookup) { this._encodingBufferOutputStream = new EncodingBufferOutputStream(); this._encodingBuffer = new byte[512];
...
/* 2375 */         Encoder.this._encodingBuffer[Encoder.this._encodingBufferIndex++] = (byte)b;
/*      */       } else {
...
/*      */         
/* 2381 */         Encoder.this._encodingBuffer[Encoder.this._encodingBufferIndex++] = (byte)b;
/*      */       } 
/*      */     }
/*      */     
...
/* 2451 */       if (c < 128) {
/*      */         
/* 2453 */         this._encodingBuffer[bpos++] = (byte)c; continue;
/* 2454 */       }  if (c < 2048) {
/*      */         
/* 2456 */         this._encodingBuffer[bpos++] = (byte)(0xC0 | c >> 6);
/*      */         
/* 2458 */         this._encodingBuffer[bpos++] = (byte)(0x80 | c & 0x3F); continue;
/*      */       } 
/* 2460 */       if (c <= 65535) {
...
/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/xml/internal/fastinfoset/Encoder.class
