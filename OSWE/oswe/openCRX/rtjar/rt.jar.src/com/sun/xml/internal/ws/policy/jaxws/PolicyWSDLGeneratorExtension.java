...
/*     */       }
/*     */       
/* 115 */       this.policyMap = PolicyResolverFactory.create().resolve(new PolicyResolver.ServerContext(this.policyMap, context
/* 116 */             .getContainer(), context.getEndpointClass(), false, (PolicyMapMutator[])extenders));
/*     */       
...
/*     */       try {
/* 125 */         Collection<PolicySubject> policySubjects = new LinkedList<>();
/* 126 */         for (int j = 0; j < policyMapConfigurators.length; j++) {
/* 127 */           policySubjects.addAll(policyMapConfigurators[j].update(this.policyMap, this.seiModel, binding));
/* 128 */           extenders[j].disconnect();
/*     */         } 
/* 130 */         PolicyMapUtil.insertPolicies(this.policyMap, policySubjects, this.seiModel.getServiceQName(), this.seiModel.getPortName());
/* 131 */       } catch (PolicyException e) {
...
/* 152 */         PolicyModelGenerator generator = ModelGenerator.getGenerator();
/* 153 */         Set<String> policyIDsOrNamesWritten = new HashSet<>();
/* 154 */         for (PolicySubject subject : this.subjects) {
/* 155 */           Policy policy; if (subject.getSubject() == null) {
...
/* 310 */               if (exceptionName.equals(fault.getName()) && operation
/* 311 */                 .getName().getLocalPart().equals(method.getOperationName()))
/* 312 */                 writePolicyOrReferenceIt(subject, writer); 
/*     */               continue;
/*     */             } 
/* 315 */             if (WsdlBindingSubject.class.isInstance(concreteSubject)) {
/* 316 */               WsdlBindingSubject wsdlSubject = (WsdlBindingSubject)concreteSubject;
/* 317 */               if (wsdlSubject.getMessageType() == WsdlBindingSubject.WsdlMessageType.FAULT && exception
/* 318 */                 .getOwner().getTargetNamespace().equals(wsdlSubject.getName().getNamespaceURI()) && exceptionName
/* 319 */                 .equals(wsdlSubject.getName().getLocalPart())) {
...
/* 380 */               writePolicyOrReferenceIt(subject, xmlWriter); continue;
/*     */             } 
/*     */             try {
...
/* 454 */     Collection<PolicyMapConfigurator> configurators = new LinkedList<>();
/*     */ 
/*     */     
...
/*     */     
/* 463 */     return configurators.<PolicyMapConfigurator>toArray(new PolicyMapConfigurator[configurators.size()]);
/*     */   }
/*     */ }


/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/xml/internal/ws/policy/jaxws/PolicyWSDLGeneratorExtension.class
