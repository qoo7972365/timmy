...
/*      */       }
/*      */     } 
/*  472 */     this.compute = computeFrames ? 0 : (computeMaxs ? 1 : 2);
/*  473 */     if (computeMaxs || computeFrames) {
/*  474 */       if (computeFrames && "<init>".equals(name)) {
/*  475 */         this.access |= 0x40000;
/*      */       }
/*      */       
...
/*      */     
/*  581 */     if (type == -1) {
/*  582 */       startFrame(this.code.length, nLocal, nStack); int i;
/*  583 */       for (i = 0; i < nLocal; i++) {
/*  584 */         if (local[i] instanceof String) {
/*  585 */           this.frame[this.frameIndex++] = 0x1700000 | this.cw
/*  586 */             .addType((String)local[i]);
/*  587 */         } else if (local[i] instanceof Integer) {
/*  588 */           this.frame[this.frameIndex++] = ((Integer)local[i]).intValue();
/*      */         } else {
/*  590 */           this.frame[this.frameIndex++] = 0x1800000 | this.cw
/*  591 */             .addUninitializedType("", ((Label)local[i]).position);
/*      */         } 
/*      */       } 
/*      */       
/*  595 */       for (i = 0; i < nStack; i++) {
/*  596 */         if (stack[i] instanceof String) {
/*  597 */           this.frame[this.frameIndex++] = 0x1700000 | this.cw
/*  598 */             .addType((String)stack[i]);
/*  599 */         } else if (stack[i] instanceof Integer) {
/*  600 */           this.frame[this.frameIndex++] = ((Integer)stack[i]).intValue();
/*      */         } else {
/*  602 */           this.frame[this.frameIndex++] = 0x1800000 | this.cw
/*  603 */             .addUninitializedType("", ((Label)stack[i]).position);
/*      */         } 
/*      */       } 
/*      */       
/*  607 */       endFrame();
...
/*  617 */       switch (type) {
/*      */         case 0:
/*  619 */           this.stackMap.putByte(255)
/*  620 */             .putShort(delta)
/*  621 */             .putShort(nLocal);
/*  622 */           for (i = 0; i < nLocal; i++) {
/*  623 */             writeFrameType(local[i]);
/*      */           }
/*  625 */           this.stackMap.putShort(nStack);
/*  626 */           for (i = 0; i < nStack; i++) {
/*  627 */             writeFrameType(stack[i]);
/*      */           }
/*      */           break;
/*      */         case 1:
/*  631 */           this.stackMap.putByte(251 + nLocal)
/*  632 */             .putShort(delta);
...
/*      */           break;
/*      */         case 2:
/*  638 */           this.stackMap.putByte(251 - nLocal)
/*  639 */             .putShort(delta);
/*      */           break;
/*      */         case 3:
/*  642 */           if (delta < 64) {
/*  643 */             this.stackMap.putByte(delta); break;
/*      */           } 
...
/*      */         
/*  721 */         this.currentBlock.status |= 0x100;
/*      */ 
/*      */         
...
/*      */         
/*  893 */         (label.getFirst()).status |= 0x10;
/*      */         
...
/*  903 */           label.status |= 0x200;
/*  904 */           this.subroutines++;
/*      */         } 
/*  906 */         this.currentBlock.status |= 0x80;
...
/* 1132 */         (dflt.getFirst()).status |= 0x10;
/* 1133 */         for (int i = 0; i < labels.length; i++) {
/* 1134 */           addSuccessor(0, labels[i]);
/* 1135 */           (labels[i].getFirst()).status |= 0x10;
/*      */         } 
/*      */       } else {
/*      */         
/* 1139 */         this.stackSize--;
...
/*      */       }
/* 1200 */       this.localVarTypeCount++;
/* 1201 */       this.localVarType.putShort(start.position)
/* 1202 */         .putShort(end.position - start.position)
/* 1203 */         .putShort(this.cw.newUTF8(name))
/* 1204 */         .putShort(this.cw.newUTF8(signature))
/* 1205 */         .putShort(index);
/*      */     } 
/* 1207 */     if (this.localVar == null) {
/* 1208 */       this.localVar = new ByteVector();
/*      */     }
/* 1210 */     this.localVarCount++;
/* 1211 */     this.localVar.putShort(start.position)
/* 1212 */       .putShort(end.position - start.position)
/* 1213 */       .putShort(this.cw.newUTF8(name))
/* 1214 */       .putShort(this.cw.newUTF8(desc))
/* 1215 */       .putShort(index);
...
/* 1240 */         Label label1 = handler.start.getFirst();
...
/* 1255 */           b.successor = h;
/*      */           
/* 1257 */           b.next = label1.successors;
/* 1258 */           label1.successors = b;
/*      */           
/* 1260 */           label1 = label1.successor;
/*      */         } 
...
/*      */         
/* 1281 */         Label label = changed;
/* 1282 */         changed = changed.next;
/* 1283 */         label.next = null;
/* 1284 */         f = label.frame;
/*      */         
/* 1286 */         if ((label.status & 0x10) != 0) {
/* 1287 */           label.status |= 0x20;
/*      */         }
/*      */         
/* 1290 */         label.status |= 0x40;
/*      */         
/* 1292 */         int blockMax = f.inputStack.length + label.outputStackMax;
/* 1293 */         if (blockMax > max) {
/* 1294 */           max = blockMax;
/*      */         }
/*      */         
/* 1297 */         Edge e = label.successors;
/* 1298 */         while (e != null) {
...
/* 1333 */             this.frame[this.frameIndex++] = 0x1700000 | this.cw
/* 1334 */               .addType("java/lang/Throwable");
/* 1335 */             endFrame();
/*      */           } 
/*      */         } 
/* 1338 */         l = l.successor;
...
/*      */               
/* 1388 */               id++;
/* 1389 */               subroutine.visitSubroutine(null, id / 32L << 32L | 1L << id % 32, this.subroutines);
/*      */             } 
/*      */           } 
/*      */           
/* 1393 */           l = l.successor;
...
/*      */             
/* 1446 */             l.inputStackTop = (b.info == Integer.MAX_VALUE) ? 1 : (start + b.info);
/*      */ 
/*      */             
/* 1449 */             l.status |= 0x8;
/* 1450 */             l.next = stack;
...
/* 1487 */         return n << 2 | ((car == 'V') ? 0 : ((car == 'D' || car == 'J') ? 2 : 1));
/*      */       } 
/* 1489 */       if (car == 'L') {
...
/* 1641 */     int delta, clocalsSize = this.frame[1];
...
/*      */         return;
/*      */       case 247:
/* 1702 */         this.stackMap.putByte(247)
/* 1703 */           .putShort(delta);
...
/*      */     } 
/*      */     
/* 1718 */     this.stackMap.putByte(255)
/* 1719 */       .putShort(delta)
...
/*      */           case 24117248:
/* 1744 */             this.stackMap.putByte(7)
/* 1745 */               .putShort(this.cw.newClass((this.cw.typeTable[v]).strVal1));
/*      */             break;
/*      */           case 25165824:
/* 1748 */             this.stackMap.putByte(8).putShort((this.cw.typeTable[v]).intVal);
/*      */             break;
/*      */           default:
/* 1751 */             this.stackMap.putByte(v); break;
/*      */         } 
/*      */       } else {
...
/* 2033 */       out.putShort(this.cw.newUTF8("Exceptions"))
/* 2034 */         .putInt(2 * this.exceptionCount + 2);
...
/* 2049 */       out.putShort(this.cw.newUTF8("Signature"))
/* 2050 */         .putInt(2)
/* 2051 */         .putShort(this.cw.newUTF8(this.signature));
/*      */     }
...
/* 2142 */         int label, newOffset, opcode = b[u] & 0xFF;
/* 2143 */         int insert = 0;
/*      */         
...
/*      */     
/* 2284 */     int j = 0;
/* 2285 */     while (j < this.code.length) {
/* 2286 */       int v, label, k, newOffset, opcode = b[j] & 0xFF;
...
/* 2299 */             label = j + readUnsignedShort(b, j + 1);
/*      */           } else {
/* 2301 */             label = j + readShort(b, j + 1);
/*      */           } 
/* 2303 */           newOffset = getNewOffset(allIndexes, allSizes, j, label);
/* 2304 */           if (resize[j]) {
/*      */ 
/*      */ 
/*      */ 
...
/*      */           continue;
/*      */         case 9:
/* 2331 */           label = j + readInt(b, j + 1);
/* 2332 */           newOffset = getNewOffset(allIndexes, allSizes, j, label);
...
/* 2344 */           label = v + readInt(b, j);
/* 2345 */           j += 4;
/* 2346 */           newOffset = getNewOffset(allIndexes, allSizes, v, label);
/* 2347 */           newCode.putInt(newOffset);
/* 2348 */           k = readInt(b, j);
/* 2349 */           j += 4;
/* 2350 */           newCode.putInt(k);
/* 2351 */           k = readInt(b, j) - k + 1;
/* 2352 */           j += 4;
/* 2353 */           newCode.putInt(readInt(b, j - 4));
/* 2354 */           for (; k > 0; k--) {
/* 2355 */             label = v + readInt(b, j);
/* 2356 */             j += 4;
...
/* 2368 */           label = v + readInt(b, j);
/* 2369 */           j += 4;
/* 2370 */           newOffset = getNewOffset(allIndexes, allSizes, v, label);
/* 2371 */           newCode.putInt(newOffset);
/* 2372 */           k = readInt(b, j);
/* 2373 */           j += 4;
/* 2374 */           newCode.putInt(k);
/* 2375 */           for (; k > 0; k--) {
/* 2376 */             newCode.putInt(readInt(b, j));
/* 2377 */             j += 4;
/* 2378 */             label = v + readInt(b, j);
/* 2379 */             j += 4;
...
/*      */           continue;
/*      */         case 16:
/* 2385 */           opcode = b[j + 1] & 0xFF;
/* 2386 */           if (opcode == 132) {
/* 2387 */             newCode.putByteArray(b, j, 6);
/* 2388 */             j += 6; continue;
/*      */           } 
/* 2390 */           newCode.putByteArray(b, j, 4);
/* 2391 */           j += 4;
/*      */           continue;
/*      */         
/*      */         case 1:
/*      */         case 3:
/*      */         case 10:
/* 2397 */           newCode.putByteArray(b, j, 2);
/* 2398 */           j += 2;
/*      */           continue;
/*      */         case 2:
/*      */         case 5:
...
/*      */         case 11:
/*      */         case 12:
/* 2405 */           newCode.putByteArray(b, j, 3);
/* 2406 */           j += 3;
/*      */           continue;
/*      */         case 7:
/* 2409 */           newCode.putByteArray(b, j, 5);
/* 2410 */           j += 5;
/*      */           continue;
/*      */       } 
/*      */       
/* 2414 */       newCode.putByteArray(b, j, 4);
/* 2415 */       j += 4;
/*      */     } 
/*      */ 
/*      */ 
...
/*      */           
/* 2438 */           j = l.position - 3;
/* 2439 */           if ((l.status & 0x20) != 0 || (j >= 0 && resize[j])) {
/*      */             
...
/* 2478 */         b = bv.data;
/* 2479 */         j = 0;
/* 2480 */         while (j < bv.length) {
/* 2481 */           int label = readUnsignedShort(b, j);
/* 2482 */           int newOffset = getNewOffset(allIndexes, allSizes, 0, label);
/* 2483 */           writeShort(b, j, newOffset);
/* 2484 */           label += readUnsignedShort(b, j + 2);
/* 2485 */           newOffset = getNewOffset(allIndexes, allSizes, 0, label) - newOffset;
/*      */           
/* 2487 */           writeShort(b, j + 2, newOffset);
/* 2488 */           j += 10;
/*      */         } 
/*      */       } 
/*      */     } 
...
/* 2494 */       j = 0;
/* 2495 */       while (j < this.lineNumber.length) {
/* 2496 */         writeShort(b, j, getNewOffset(allIndexes, allSizes, 0, 
/*      */ 
/*      */               
/* 2499 */               readUnsignedShort(b, j)));
/* 2500 */         j += 4;
/*      */       } 
/*      */     } 
/*      */     
...
/* 2628 */       label.status |= 0x4;
/*      */     } 
/*      */   }
/*      */ }


/* Location:              /Users/timmy/timmy/OSWE/oswe/openCRX/rt.jar!/com/sun/xml/internal/ws/org/objectweb/asm/MethodWriter.class
