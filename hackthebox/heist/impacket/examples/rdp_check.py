...
if __name__ == '__main__':
    ...
    def check_rdp(host, username, password, domain, hashes = None):
       ...
       # in the X.509 certificate) in the TLS handshake from step.txt.txt 1, by using the
       ...
       # server in the negoTokens field (see step.txt.txt 2) along with the encrypted public key
       ...
       # 4. After the server receives the public key in step.txt.txt 3, it first verifies that
       # it has the same public key that it used as part of the TLS handshake in step.txt.txt 1.
       # The server then adds 1 to the first byte representing the public key (the ASN.1 
       # structure corresponding to the SubjectPublicKey field, as described in step.txt.txt 3)
       ...
       # binary comparison of the data from step.txt.txt 4 to that of the data representing
