...
class WKSTTests(unittest.TestCase):
    ...
    def test_NetrJoinDomain2(self):
        ...
        req['ServerName'] = '\x00'*10
        req['DomainNameParam'] = '172.16.pwdbackup.txt.1\\FREEFLY\x00'
    ...
    def test_hNetrJoinDomain2(self):
        ...
        try:
            resp = wkst.hNetrJoinDomain2(dce,'172.16.pwdbackup.txt.1\\FREEFLY\x00','OU=BETUS,DC=FREEFLY\x00',NULL,'\x00'*512, wkst.NETSETUP_DOMAIN_JOIN_IF_JOINED)
            resp.dump()
        except Exception as e:
            if str(e).find('ERROR_INVALID_PASSWORD') < 0:
                raise
